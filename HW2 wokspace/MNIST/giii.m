...
for j=1:500
    %forward activation.
    z(:,2:numofhidden)=max(train * w1,zeros(numoftrain,numofhidden-1));
    ...
    acc_h=[acc_h predict(numofhidden,holdon,w1,w2,hold_labels,'relu' )];
    acc_t=[acc_t predict(numofhidden,test,w1,w2,test_labels,'relu' )];
    pred=count/length(y)
    acc=[acc pred];
    %back propagation.
    w2=w2+alpha.* (z'*(trl-y));
    f=max(zeros(numoftrain,numofhidden-1),train*w1);
    f(f>0)=1;
    backward=f.*((trl-y)*(w2(2:numofhidden,:))'); 
